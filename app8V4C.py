...
# ==============================================================================
# MASTER PROMPT LOADER (Struktur ala Pak Felix)
# Master prompt disimpan di file markdown terpisah agar:
# - Dapat dibawa ke AI lain (ChatGPT/Claude/Manus) untuk cross-validation
# - Mudah direvisi tanpa sentuh kode
# - Sebagai artefak "Prompt Engineering Design" di Bab 3 skripsi
# ==============================================================================
MASTER_PROMPT_PATH = Path(__file__).parent / "master_prompt_inamikro.md"

def load_master_prompt():
    """Baca master prompt dari file. Jika tidak ada, return fallback minimal."""
    try:
...
try:
    # Agent Generator — untuk menghasilkan iklan
    llm_generator = ChatVertexAI(
        model_name="gemini-2.5-pro",
        temperature=0.3,
        max_output_tokens=8192
    )
    # Agent Evaluator — persona terpisah, suhu lebih rendah untuk objektivitas
    llm_evaluator = ChatVertexAI(
        model_name="gemini-2.5-pro",
        temperature=0.1,
        max_output_tokens=4096
    )
    # Agent Cross-Evaluator — model Flash sebagai "second opinion"
...
def build_context_block(kategori, nama_produk, keywords_list, gaya, platform,
    ...
    """Bangun blok konteks input terstruktur yang akan disuntikkan ke master prompt."""
...
- Tipe UMKM: {KBLI_DATA.get(kategori, {}).get('tipe', 'food')}
...
def generate_ad_text_master(
    ...
    """
    Generate iklan menggunakan Master Prompt + konteks terstruktur.
    Master prompt = blueprint umum. Context block = data konkret user saat ini.
    """
...
def generate_ad_revision_master(
    ...
    """Revisi iklan dengan acuan Master Prompt (mode Chatbot)."""
    context_block = build_context_block(
        kategori, nama_produk, keywords_list, gaya, platform,
        market, mood, background, subjek, elemen_wajib
    )
...
def build_evaluation_prompt(nama_produk, keywords_list, target_market, platform,
    ...
    """Bangun prompt evaluasi dengan persona Juri terpisah."""
    keywords_str = ", ".join(keywords_list) if keywords_list else nama_produk
    elemen_str = "\n".join([f"  - {e}" for e in elemen_wajib])
...
ANDA BUKAN AGEN YANG MEMBUAT IKLAN INI. Anda adalah evaluator independen.
...
def evaluate_ad_quality_dual(nama_produk, keywords_list, target_market, platform,
    ...
    """
    Dual AI Expert Review (Cross-AI Validation) — sesuai arahan Pak Felix.
    Menggunakan 2 model berbeda sebagai juri:
    1. Gemini 2.5 Pro (high reasoning)
    2. Gemini 2.5 Flash (second opinion)
    """
...
def extract_total_score(review_text):
    """Ekstrak total skor (XX/100) dari output evaluator untuk komparasi."""
    try:
...
# ==============================================================================
# BAGIAN 4: UI/UX STREAMLIT — V7 FINAL
# ==============================================================================

st.set_page_config(
    page_title="Inamikro Ad Generator V7",
    layout="wide",
    page_icon="📈"
)
...
<div class="main-header">
    ...
    <p>Platform Konten Iklan UMKM Berbasis Agentic AI — Gemini 2.5 Pro & Imagen 3 &nbsp;|&nbsp; Versi 7 (Master Prompt Edition)</p>
</div>
""", unsafe_allow_html=True)

# Badge info Master Prompt
col_badge1, col_badge2 = st.columns([3, 1])
with col_badge1:
    mp_status = "✅ Master Prompt aktif" if MASTER_PROMPT_PATH.exists() else "⚠️ Master Prompt fallback (file tidak ditemukan)"
    st.markdown(f'<div class="master-prompt-badge">🧠 {mp_status} | File: <code>master_prompt_inamikro.md</code></div>', unsafe_allow_html=True)
with col_badge2:
...
with col_result:
    ...
    if st.session_state.main_text_result:
        with st.container(border=True):
            ...
            # DUAL AI EXPERT REVIEW (Pak Felix: "dinilai dengan tools yang lain")
            col_ev_btn, col_ev_info = st.columns([1.3, 1])
            with col_ev_btn:
                if st.button("🤖 Minta Penilaian 2 AI Evaluator (Dual Review)", use_container_width=True):
                    with st.spinner("Dua juri AI sedang mengevaluasi (Gemini Pro + Flash)..."):
        ...
        if st.session_state.ai_review_pro or st.session_state.ai_review_flash:
            ...
            # Detail masing-masing evaluator
        ...
        with st.expander("✏️ Tambahkan Teks/Tulisan ke Gambar (Opsional)"):
            st.caption("Misal: harga, tagline. Maks 35 karakter.")
    ...
    if st.session_state.image_result_final:
        with st.container(border=True):
            st.markdown("#### 🖼️ Hasil Visual Final")
    ...
    if st.session_state.main_text_result:
        ...
        st.caption(
            "Kurang puas? Ketik instruksi revisi. AI memperbarui teks **dan** prompt visual Langkah 5."
        )
