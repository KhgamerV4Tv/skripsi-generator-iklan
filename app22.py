...
# ==============================================================================
st.set_page_config(
    page_title="Ad Generator V21 Pro (Gemini Edition)",
    layout="wide",
    page_icon="📈",
...
<div class="hero-header">
    <div class="hero-content">
        ...
        <div class="hero-text">
            <h1> Ad Generator <span style="color: #fbbf24;">V21 Pro</span></h1>
            <div class="hero-sub">Platform Generator Copywriting & Komparasi Engine Visual Skripsi UMKM (Gemini Engine)</div>
        </div>
    </div>
...
# ==============================================================================
def get_active_step():
    if st.session_state.get('main_txt') and st.session_state.img_mem.get('A'): return 6
    if st.session_state.get('main_txt'): return 5
    return 3

active_step = get_active_step()

steps_data = [("1", "Branding"), ("2", "Produk"), ("3", "Platform"), ("4", "Copywriting"), ("5", "Visual"), ("6", "Revisi")]
stepper_html = '<div class="stepper-wrap"><div class="stepper">'
for i, (num, label) in enumerate(steps_data, 1):
    step_num = int(num)
    if step_num < active_step: cls_circle, cls_label, icon = "step-circle done", "step-label-txt", "✓"
    elif step_num == active_step: cls_circle, cls_label, icon = "step-circle active", "step-label-txt active", num
    else: cls_circle, cls_label, icon = "step-circle", "step-label-txt", num
...
# --- KOLOM KIRI: INPUT DATA TERSTRUKTUR ---
with col_f:
    ...
    with st.container(border=True):
        ...
        with col_tm1:
            ...
            if "Lainnya (Ketik Manual)" in lokasi_raw:
                lokasi_manual = st.text_input("📝 Ketik Lokasi Lainnya", placeholder="Contoh: Bali, Makassar")
                if lokasi_manual: lokasi_final.append(lokasi_manual)
            umur_raw = st.multiselect("🎂 Umur", ["Semua Umur", "Anak-anak", "Remaja (13-18 thn)", "Dewasa Muda (19-35 thn)", "Dewasa (36-50 thn)"], default=["Semua Umur"])
        with col_tm2:
            gender = st.selectbox("🚻 Gender", ["Semua Gender", "Perempuan Khusus", "Laki-laki Khusus"])
            pasar_raw = st.multiselect("👥 Kategori Target Pasar", ["Umum", "Pelajar / Mahasiswa", "Pekerja Kantoran", "Ibu Rumah Tangga", "Pengusaha / Pebisnis", "Lainnya (Ketik Manual)"], default=["Umum"])
            pasar_final = [p for p in pasar_raw if p != "Lainnya (Ketik Manual)"]
            if "Lainnya (Ketik Manual)" in pasar_raw:
                pasar_manual = st.text_input("📝 Ketik Target Pasar Lainnya", placeholder="Contoh: Gamers, Coffee Lovers")
                if pasar_manual: pasar_final.append(pasar_manual)
        market = [f"Lokasi: {', '.join(lokasi_final)}", f"Gender: {gender}", f"Umur: {', '.join(umur_raw)}", f"Target Pasar: {', '.join(pasar_final)}"]
    ...
    with st.container(border=True):
        ...
        if kategori_raw == "00000 - Kategori Usaha Lainnya":
            kategori_manual = st.text_input("📝 Tuliskan Kategori Usaha Anda Secara Spesifik", placeholder="Contoh: Jasa Pembuatan Website, Bengkel Las")
            if kategori_manual: kategori_final = f"Kustom: {kategori_manual}"
        ...
        mode_promo = st.radio("💸 Metode Penginputan Harga & Promo", ["Deskripsi Bebas (Harga Sama/Massal)", "Input Item Manual Satu-per-Satu"])
        
        deskripsi_harga_global = ""
        if mode_promo == "Deskripsi Bebas (Harga Sama/Massal)":
            st.markdown("##### 🌍 Deskripsi Harga & Promo")
            deskripsi_harga_global = st.text_area("Tuliskan detail harga dan promo Anda", placeholder="Contoh: Mulai 15 ribuan aja. Promo khusus hari Jumat beli 2 gratis 1.", height=90)
        else:
            st.markdown("##### ➕ Input Item Manual Satu-per-Satu")
            c_p1, c_p2, c_p3 = st.columns([1.5, 1.2, 1.3])
            with c_p1: item_nama = st.text_input("Nama Item", placeholder="Contoh: Kopi Susu", key="input_item_nama")
            with c_p2: item_harga = st.text_input("Harga / Range Harga", placeholder="Contoh: 15rb", key="input_item_harga")
    ...
    st.markdown('<div class="section-card-header"><div class="section-num">3</div><div><div class="section-title">Strategi Platform & Format</div><div class="section-subtitle">Platform target, tone, dan mood visual</div></div></div>', unsafe_allow_html=True)
    with st.container(border=True):
        ...
        with col4: mood = st.selectbox("🌅 Mood Visual", ["Cerah & Ceria (Bright & Cheerful)", "Gelap Elegan", "Hangat & Nyaman", "Sinematik & Dramatis"])
    ...
    if st.button("🚀 GENERATE IKLAN OTOMATIS", type="primary", use_container_width=True):
        if not brand_name: st.warning("⚠️ Mohon isi nama brand terlebih dahulu!")
        else:
            ...
            with st.spinner(f"🤖 Menghubungkan naskah via Gemini 2.5 Pro..."):
                ...
                catat_aktivitas_sistem(f"Generate via Gemini 2.5 Pro", brand_name)

            with st.spinner("⚖️ Agen QC Evaluator: Menilai kelayakan kriteria promosi..."):
                st.session_state.ai_eval_result = evaluate_ad_quality_master(kategori_final, txt)
            st.rerun()

# --- KOLOM KANAN: OUTPUT, QUALITY CONTROL EVALUATOR, & RENDER ENGINE ---
with col_r:
    ...
    if st.session_state.main_txt:
        with st.container(border=True):
            ...
            if edit_mode:
                st.session_state.main_txt = st.text_area("Edit Teks Copywriting", value=st.session_state.main_txt, height=350, label_visibility="collapsed")
            else:
                st.markdown(st.session_state.main_txt)
                with st.expander("📋 Tampilkan Teks Mentah"): st.code(st.session_state.main_txt, language="markdown")
        ...
        with st.container(border=True):
            st.caption("💬 Kurang pas? Ketik perintah revisi — misal: *'Tambahkan nomor WA 08123'*")
            ...
            if revisi_input:
                ...
                with st.spinner("🧠 Menyesuaikan konteks naskah..."):
                    ...
                    st.session_state.img_mem["A"] = None
                    st.session_state.chat_history.append({"role": "assistant", "content": "✅ **Revisi selesai!** Hasil teks dan instruksi gambar sudah saya perbarui."})
                    st.rerun()

        # ==============================================================================
        # FORM PENILAIAN UAT & ADMIN AREA PANEL DARI APP 15
        # ==============================================================================
            ...
            if st.form_submit_button("📁 Simpan Penilaian UAT", use_container_width=True, type="primary"):
                ...
                if db:
                    try:
                        ...
                        st.toast("✅ Penilaian tersimpan di Cloud Firestore!", icon="💾")
        ...
        with st.expander("🔐 Menu Admin & Database Log"):
            ...
            if admin_pin == "12345678":
                ...
                with col_s1: st.markdown(f"**Koneksi Database:**<br>{db_emoji} {db_status_text}", unsafe_allow_html=True)
                with col_s2: st.markdown(f"**Total Trafik Sistem:**<br>⚡ {len(final_usage_list)} Interaksi AI", unsafe_allow_html=True)
                st.markdown("<hr>", unsafe_allow_html=True)

                st.markdown("#### 📡 Log Interaksi AI (Traffic)")
                if final_usage_list:
                    df_usage = pd.DataFrame(final_usage_list)
                    if "Waktu" in df_usage.columns: df_usage = df_usage.sort_values(by="Waktu", ascending=False).reset_index(drop=True)
                    st.dataframe(df_usage, use_container_width=True, hide_index=True, height=200)
                else: st.info("Belum ada log penggunaan.")
